export type CameraConnection = { stop(): void };
export async function connectCamera(token: string, role: 'phone' | 'viewer', stream: MediaStream | null, onStream: (stream: MediaStream) => void, onStatus: (status: string) => void): Promise<CameraConnection> {
  ...
  const connect = () => {
    ...
    let offering = false;
    const offer = async () => { if (role !== 'phone' || offering) return; offering = true; try { await pc.setLocalDescription(await pc.createOffer()); send({ type: 'offer', sdp: pc.localDescription!.sdp }); } finally { offering = false; } };
    ws.onmessage = async event => {
      try {
        const message = JSON.parse(event.data);
        if (message.type === 'ready') { attempts = 0; onStatus('Waiting for peer'); if (message.peerConnected) await offer(); }
        if (message.type === 'peer-connected') await offer();
        ...
        if (message.type === 'peer-disconnected' || message.type === 'stop') { onStatus('Peer disconnected'); pc.close(); ws.close(); }
      } catch (error) { onStatus(`Connection error: ${String(error)}`); }
    };
    ws.onclose = event => { pc.close(); if (stopped) return; if (event.code === 4003 || event.code === 4001 || event.code === 1008 || ++attempts > 5) { onStatus(event.reason || 'Pairing unavailable or expired. Create a new pairing.'); stream?.getTracks().forEach(track => track.stop()); return; } onStatus('Disconnected — retrying'); retry = setTimeout(connect, Math.min(10_000, 1500 * attempts)); };
    ws.onerror = () => onStatus('Signaling unavailable; verify HTTPS and pairing link');
  };
  connect();
  return { stop() { stopped = true; clearTimeout(retry); if (ws.readyState === WebSocket.OPEN) ws.send(JSON.stringify({ type: 'stop' })); ws.close(); pc.close(); stream?.getTracks().forEach(track => track.stop()); onStatus('Stopped'); } };
}
