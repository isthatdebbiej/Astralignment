...
import type { EvaluationResult, RepairArtifact, SceneConfig, SearchResult, WorldSnapshot } from '../../contracts/index';
...
export default function App() {
  ...
  const findCounterexample = () => void execute('Searching counterexamples', async () => {
    setActiveTask('search'); setTab('evidence'); setReplayData(null); setScrubIndex(null);
  ...
  return <div className="app-shell">
    ...
    <main className={`workspace-grid ${showLeft ? '' : 'rail-collapsed'}`}>
      ...
      <div className="stage-container"><SimulationStage model={renderModel} state={displayState ?? null} connected={connected} mode={mode}/><div className="stage-metrics"><div><span className="robot-key robot-a"/> G1—A <strong>{displayState?.robots?.[0] ? displayState.robots[0].fallen ? 'FALLEN' : displayState.robots[0].goal_reached ? 'ARRIVED' : displayState.robots[0].action.toUpperCase() : 'WAITING'}</strong></div><div><span className="robot-key robot-b"/> G1—B <strong>{displayState?.robots?.[1] ? displayState.robots[1].fallen ? 'FALLEN' : displayState.robots[1].goal_reached ? 'ARRIVED' : displayState.robots[1].action.toUpperCase() : 'WAITING'}</strong></div><div className="separation-metric">MIN SEPARATION <strong>{number(displayState?.metrics.min_separation)} <small>m</small></strong></div></div></div>
      <div className="simulation-actions"><button className="button icon-only" title="Randomize spawn" aria-label="Randomize spawn" disabled={locked} onClick={randomize}><Shuffle size={16}/></button><button className="button primary" disabled={locked} onClick={snapshot?.paused !== false || replayFrames.length ? baseline : pause}>{snapshot?.paused !== false || replayFrames.length ? <Play size={14} fill="currentColor"/> : <Pause size={14}/>}<span>{snapshot?.paused !== false || replayFrames.length ? 'Run baseline' : 'Pause'}</span><kbd aria-hidden="true">␣</kbd></button><button className="button" disabled={locked} onClick={findCounterexample}><Search size={14}/><span>Find counterexample</span></button><button className="reference-button" disabled={locked || !model} aria-label="Run reference from scene start" title="Run reference from scene start with fresh metrics" onClick={reference}>Reference from start<RotateCcw size={12}/></button></div>
      <section className="timeline-panel"><div className="timeline-heading"><div><Activity size={13}/><strong>Correlated timeline</strong><span>{timelineFrames.length ? `${timelineFrames.length} captured frames` : 'Awaiting first episode'}</span></div><div><button className={`timeline-live ${!replayFrames.length && scrubIndex == null ? 'active' : ''}`} onClick={() => { setReplayData(null); setScrubIndex(null); setReplaying(false); }}><span className="status-dot"/>LIVE</button><button className="icon-button" aria-label={replaying ? 'Pause replay' : 'Play replay'} disabled={!replayFrames.length} onClick={() => setReplaying(value => !value)}>{replaying ? <Pause size={13}/> : <Play size={13}/>}</button></div></div><div className="timeline-ruler"><span>{elapsed(timelineMin)}</span><span>{elapsed(timelineMin + (timelineMax - timelineMin) / 2)}</span><span>{elapsed(timelineMax)}</span></div><div className="timeline-track"><span className="track-label">min. dist.</span><div className="chart-area"><svg viewBox="0 0 1000 44" preserveAspectRatio="none" aria-label="Running minimum robot separation"><line x1="0" y1="40" x2="1000" y2="40" stroke="currentColor" strokeDasharray="3 6"/>{separationPoints && <polyline points={separationPoints} fill="none" stroke="#a3c9bf" strokeWidth="1.6" vectorEffect="non-scaling-stroke"/>}</svg><input aria-label="Inspect captured simulation frame" type="range" min="0" max={Math.max(0, timelineFrames.length - 1)} value={replayFrames.length ? replayIndex : scrubIndex ?? Math.max(0, history.length - 1)} disabled={!timelineFrames.length} onChange={event => { const value = Number(event.target.value); if (replayFrames.length) { setReplayIndex(value); setReplaying(false); } else { setScrubIndex(value); setScrubState(history[value] ?? null); } }}/></div></div><div className="timeline-event"><span className="track-label">events</span>{displayEvents.length ? <div className="last-event"><span className={`event-point ${displayEvents.at(-1)?.severity}`}/><code>{elapsed(displayEvents.at(-1)!.sim_time)}</code><span>{displayEvents.at(-1)?.message}</span></div> : <span className="timeline-empty">Physics events will appear here as the episode runs.</span>}</div></section>
      </section>
