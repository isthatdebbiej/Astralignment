...
function CurationWorkspace(){
 ...
 const [sort,setSort]=useState(initial.get('sort')??'relevance');
 const [applied,setApplied]=useState<{text:string;mode:string;filters:Record<string,string>;sort:string}|null>(null);
 const [coverage,setCoverage]=useState(''),[total,setTotal]=useState(0);
 const searchRequest=useRef(0),returnEpisode=useRef<string|null>(null);
 async function refresh(){const [s,c,j,configuration]=await Promise.all([api('/sources?offset='+sourceOffset+'&project_id='+encodeURIComponent(project)),api('/collections?offset='+collectionOffset+'&project_id='+encodeURIComponent(project)),api('/jobs?offset='+jobOffset),api('/settings')]);setSources(s);setCollections(c);setJobs(j);setSettings(configuration);}
 async function search(start=0,query={text,mode,filters,sort}){const request=++searchRequest.current;const result=await api('/queries',{text:query.text,mode:query.mode,sort:query.sort,project_id:project,offset:start,...Object.fromEntries(Object.entries(query.filters).filter(([,v])=>v))});if(request!==searchRequest.current)return;setEpisodes(result.episodes);setApplied(query);setCoverage(result.coverage);setTotal(result.total);setOffset(start);}
 ...
 url.searchParams.set('view',page);url.searchParams.set('project',project);url.searchParams.set('mode',applied?.mode??'metadata');url.searchParams.set('q',applied?.text??'');url.searchParams.set('sort',applied?.sort??'relevance');url.searchParams.set('offset',String(offset));
 for(const [key,value] of Object.entries(applied?.filters??{})){if(value)url.searchParams.set(key,value);else url.searchParams.delete(key);}
 if(selected)url.searchParams.set('episode',selected.id);else {url.searchParams.delete('episode');url.searchParams.delete('frame');url.searchParams.delete('stream');for(const prefix of ['selection','review'])for(const key of ['scope','stream','start','end'])url.searchParams.delete(prefix+'_'+key);}
 if(url.searchParams.get('collection')!==collection?.id)url.searchParams.delete('version');
 ...
 },[hydrated,page,project,applied,offset,selected?.id,collection?.id]);
 useEffect(()=>{if(selected){returnEpisode.current=selected.id;document.getElementById('episode-heading')?.focus();}else if(returnEpisode.current&&page==='library'){document.querySelector<HTMLElement>('[data-episode="'+CSS.escape(returnEpisode.current)+'"]')?.focus();returnEpisode.current=null;}},[selected?.id,page]);
 ...
 {settings&&<><p>CPU workers: {settings.cpu_worker_count}</p><p>Import limit: {(settings.import_limit_bytes/1073741824).toFixed(1)} GiB</p><p>Source access: registered local snapshots only; original media is not bundled in exports.</p><p>Metadata limit: {(settings.metadata_limit_bytes/1048576).toFixed(0)} MiB</p><p>Export limit: {(settings.export_limit_bytes/1048576).toFixed(0)} MiB</p><p>Available filesystem: {(settings.filesystem_available_bytes/1073741824).toFixed(1)} GiB</p><small>{settings.backup}</small></>}
 ...
 {settings?.worker&&settings.worker.status!=='available'&&<div role="status" className="cu-alert">Worker: {settings.worker.status}. {settings.worker.message} {settings.worker.last_contact&&<>Last contact: {settings.worker.last_contact}. </>}{settings.worker.can_restart&&<button disabled={busy} onClick={()=>void run(async()=>{await api('/worker/restart',{});})}>Restart worker</button>}</div>}
 ...
 {selected?<section className="cu-panel"><button onClick={()=>setSelected(null)}>← Back to Library</button><h2 id="episode-heading" tabIndex={-1}>{selected.task} / {selected.source_episode}</h2>
 ...
 <EpisodeEvidence key={'evidence-'+selected.id} episode={selected} reviews={selected.reviews}/>
 ...
 </div><IntervalFields episode={selected} prefix="review"/>
 ...
 <label>Sort results<select value={sort} onChange={e=>setSort(e.target.value)}><option value="relevance">Relevance</option><option value="task">Task</option><option value="split">Official split</option></select></label>
 <p className="cu-muted">Showing executed search “{applied?.text??''}”, sorted by {applied?.sort??'relevance'}. {applied?.mode==='annotations'?'Includes source task text, source labels and reviewer judgments. Not automatic failure detection.':'Searches task identifiers, robot and split. Not label-hidden prediction.'} {coverage}</p>
 {applied&&JSON.stringify(applied)!==JSON.stringify({text,mode,filters,sort})&&<p role="status">Search controls have unapplied changes. Search or Apply filters to update results.</p>}
 ...
 <section className="cu-panel"><div className="cu-table-head"><h2>Episodes</h2><span>{episodes.length} on this page · {total} matching indexed episodes</span></div>
 ...
 await api('/collections/'+c.id,{revision:c.revision,members:[...c.members,{episode_id:e.id,interval:null}],exclusions:c.exclusions,selection:{...c.selection,[e.id]:{...applied,selected_at:new Date().toISOString(),method:'library-search'}}},'PATCH');setNotice('Added to '+c.name);})}>Add to collection</button></td></tr>)}</tbody></table></div>}
 <footer><button disabled={offset===0||busy} onClick={()=>void run(()=>search(Math.max(0,offset-40),applied??undefined))}>Previous</button><button disabled={offset+episodes.length>=total||busy} onClick={()=>void run(()=>search(offset+40,applied??undefined))}>Next</button></footer></section>
 <details className="cu-panel" open={!episodes.length}><summary>Sources ({sources.length} on this page)</summary><footer><button disabled={sourceOffset===0} onClick={()=>setSourceOffset(Math.max(0,sourceOffset-40))}>Previous sources</button><button disabled={sources.length<40} onClick={()=>setSourceOffset(sourceOffset+40)}>Next sources</button></footer>{sources.map(s=><article key={s.id}><h3>{s.name}</h3><p>{s.status} · {s.tasks.join(', ')}</p>{s.quality&&<p>{s.quality.imported} published logical episodes · {s.quality.with_findings} with integrity findings · {s.quality.planned==null?'preflight pending':Math.max(0,s.quality.planned-s.quality.imported)+' not yet published'}. Published episodes remain inspectable during interrupted imports.</p>}<button disabled={busy} onClick={()=>void run(async()=>{await api('/sources/'+s.id+'/revisions',{});setNotice('New source revision queued; existing evidence remains unchanged.');})}>Reinspect as new revision</button>
 {s.plan&&<><p>{s.plan.estimated_bytes.toLocaleString()} selected bytes · {s.plan.files.length} files</p><p>Adapter: BotFails LeRobot v2.0. Preflight verifies local file identities; media/channel integrity is checked during import.</p><p>Derived-space reserve: at least {Math.max(64*1024*1024,Math.floor(s.plan.estimated_bytes/20)).toLocaleString()} bytes (admission estimate, not measured usage). Import limit: {settings?.import_limit_bytes?.toLocaleString()} bytes. Available filesystem: {settings?.filesystem_available_bytes?.toLocaleString()} bytes.</p><p>{s.plan.findings.join('; ')}</p><details><summary>Selected files and provenance</summary><p>Revision {s.revision}</p><p>License: {s.license}</p><p>Verification: {s.plan.verification?.mode??'local-only'}</p><p>{s.plan.episodes.length} logical episodes planned</p><ul>{s.plan.files.map(f=><li key={f.path}>{f.path} · {f.bytes} bytes</li>)}</ul></details><button disabled={busy} onClick={()=>void run(async()=>{await api('/sources/'+s.id+'/imports',{confirm_bytes:s.plan!.estimated_bytes});setNotice('Import queued. Refresh search when it completes.');})}>Import this selection</button></>}</article>)}</details></>:<>
