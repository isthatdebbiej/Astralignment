...
def controller_source():
    return {"language": "python", "source": inspect.getsource(SharedWorld.desired_velocities),
...
def checkpoint():
    ...
    with svc.lock:
        ...
        svc.checkpoints[checkpoint_id] = svc.world.checkpoint()
        while len(svc.checkpoints) > 16:
            svc.checkpoints.popitem(last=False)
        return {"checkpoint_id": checkpoint_id, "episode_id": svc.world.episode_id,
