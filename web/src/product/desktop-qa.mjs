...
try {
  ...
  await page.screenshot({ path: path.join(output, 'desktop-camera-first.png'), fullPage: true });
  await page.getByRole('button', { name: 'World', exact: true }).click();
  await page.waitForSelector('.three-mount canvas');
  await page.waitForFunction(() => document.querySelector('.simulation-stage')?.getAttribute('data-model-ready') === 'true' && document.querySelector('.connection-state')?.classList.contains('online'), null, { timeout: 90000 });
  ...
  if (phase === 'journey') {
    await page.getByRole('button', { name: 'More scene actions', exact: true }).click();
    ...
    await page.getByRole('dialog', { name: 'Scene setup' }).waitFor({ state: 'visible' });
    ...
    await errors.getByRole('button', { name: 'World', exact: true }).click();
