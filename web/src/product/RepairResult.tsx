...
export function RepairResult({ repair, baseline, stale, busy, progress, onReplay, onInspect }: RepairResultProps) {
  ...
  const evaluated = !stale && Boolean(repair.source_hash) && evaluation?.policy_hash === repair.source_hash;
  ...
  return <section className={`repair-result repair-result-${verdict.toLowerCase()}`} aria-label="Repair result">
    ...
    <div className="repair-result-actions"><button className="button" disabled={!terminal || !evaluated || !evaluation?.frames.length} onClick={onReplay}>Watch repaired run</button><button className="button" disabled={!repair.source} onClick={onInspect}>Inspect code</button></div>
