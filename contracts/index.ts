...
export interface ModelDescription { geoms: VisualGeom[]; scene: SceneConfig; provenance: Record<string, unknown> }
export interface RobotCommand { robot_id: RobotId; action: 'go' | 'wait'; speed?: number; waypoint?: Vec2 }
export interface EvaluationResult { episode_id: string; scene_epoch: number; mode: ControllerMode; seed: number; policy_hash?: string; passed: boolean; metrics: Metrics; frames: WorldSnapshot[]; events: TraceEvent[]; reason: string }
export interface RepairArtifact { id: string; model: string; created_at: string; scene_epoch: number; status: 'generating' | 'testing' | 'passed' | 'failed' | 'error'; source: string; source_hash: string; explanation: string; test_output: string; evaluation?: EvaluationResult; usage_usd?: number }
