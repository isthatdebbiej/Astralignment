...
DimOS is a gated Ubuntu integration, not installed or verified by these files. The local simulation runs independently within Astralignment; vendored upstream assets and adapter equations are attributed in `sim/ATTRIBUTION.md`. Gateway-to-physics execution is verified with an explicitly human-authored fixture, not a fabricated model response. Live Astra repair and source-frozen held-out results require funded API access.
