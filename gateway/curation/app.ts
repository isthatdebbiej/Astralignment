...
export function createCurationApp(store:Store, sourceRoot:string, workerToken:string) {
  ...
  app.get('/api/v1/sources',(q,r)=>{const p=pagination(q.query);r.setHeader('X-Total-Count',store.count('source'));r.json(store.page('source',p.limit,p.offset));});
  ...
  app.get('/api/v1/collections',(q,r)=>{const p=pagination(q.query);r.setHeader('X-Total-Count',store.count('collection'));r.json(store.page('collection',p.limit,p.offset));});
  ...
  app.post('/api/v1/internal/claim',(_q,r)=>{
    ...
    for(const j of jobs)if(j.status==='running'&&Date.now()-Date.parse(j.updated_at)>60000){
      ...
      j.status='queued';store.put('job',j.id,j);
    }
      ...
      if(j.kind==='preflight'){
        const result=z.object({estimated_bytes:z.number().int().nonnegative(),files:z.array(file).max(20000),
          episodes:z.array(z.object({index:z.number().int().nonnegative(),task:z.string(),info:z.record(z.string(),z.unknown()),
            row:z.record(z.string(),z.unknown()),files:z.array(z.object({kind:z.string(),path:z.string()}).strict()).max(64)}).strict()).max(1000),
          findings:z.array(z.string()).max(20000)}).strict().parse(q.body.result);
        ...
        const result=z.object({episode_id:z.literal(j.payload.episode_id),artifact_sha256:z.literal(j.payload.artifact.sha256),
          ...
          rows:z.array(z.record(z.string(),z.unknown())).max(j.payload.limit),timing:z.string().max(1000)}).strict().parse(q.body.result);
